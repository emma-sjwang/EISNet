...
class AlexNetCaffe(nn.Module):
    def __init__(self, n_classes=100, dropout=True, jigsaw_classes=31):
        ...
        self.embedding = nn.Sequential(nn.Linear(4096, 128))
    ...
    def get_params(self, base_lr, moco=False):
        if moco:
            return [{"params": chain(self.embedding.parameters(),
    ...
    #                                  self.class_classifier.parameters(),
    #                                  self.embedding.parameters()), "lr": base_lr}]
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        x = self.classifier(x)
        embedding = self.l2norm(self.embedding(x))
        out = self.class_classifier(x)
        return out, self.jigsaw_classifier(x), embedding, x  #, self.domain_classifier(d)
...
def caffenet(classes):
    ...
    state_dict = torch.load("../../Ours/pretrained/alexnet_caffe.pth.tar")
